...
describe("GenerationQueue", () => {
  it("deduplicates book ids and runs only one job at a time", async () => {
    ...
    expect(queue.enqueue({ bookId: "book-1", creatorUid: "user-1" })).toBe(true);
    expect(queue.enqueue({ bookId: "book-1", creatorUid: "user-1" })).toBe(false);
    expect(queue.enqueue({ bookId: "book-2", creatorUid: "user-2" })).toBe(true);
  ...
  it("requeues all pending user-created books on startup", async () => {
    const repository = {
      listPendingGenerationJobs: vi.fn().mockResolvedValue([
        ...
        { bookId: "book-2", creatorUid: "user-2" },
      ]),
    };
